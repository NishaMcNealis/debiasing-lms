...
from transformers import GPT2Tokenizer, GPT2LMHeadModel, GPT2DoubleHeadsModel, Trainer, TrainingArguments, DataCollatorWithPadding
...
def get_model(device='cpu', gpt2_name='gpt2', in_net=False, in_net_init_identity=True, out_net=False, out_net_init_identity=True, freeze_ln=False, freeze_pos=True,
    ...
    model = AutoModelForCausalLMWithValueHead.from_pretrained("rlhf_model").to(device)
    ...
    for name, p in model.pretrained_model.transformer.named_parameters():
        name = name.lower()
    ...
    def forward(
        ...
        **kwargs
        ...
        x = self.pretrained_model.transformer.wte(input_ids)
        ...
        transformer_outputs = self.pretrained_model.transformer(
            inputs_embeds=x,
        ...
        try:
            lm_logits = self.pretrained_model.lm_head_new(hidden_states)
        except AttributeError:
...
if __name__ == "__main__":
    ...
    #     output_dir="{}gpt2_rlhf_reward_model".format(PATH_TO_SAVE),
    #     learning_rate=9e-6,
    #     per_device_train_batch_size=4,
    #     per_device_eval_batch_size=4,
    #     num_train_epochs=2,
    #     weight_decay=0.001, #default
    ...
    dataset = load_dataset("text", data_files={"train": "combined.txt", "val": "cross_validation.txt"})
    print("Sample data: ", dataset["train"][0])
    print(dataset.column_names)
    original_columns = dataset["train"].column_names

    dataset = dataset.map(lambda examples: tokenizer(examples["text"]), batched=True, remove_columns=original_columns)    
    print("Sample tokenized data: ", dataset["train"][0])
    ...
    trainer.train()
    model.save_pretrained("./debiased_rlhf_model")
